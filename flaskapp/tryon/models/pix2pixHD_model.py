...
class Pix2PixHDModel(BaseModel):
    ...
    def forward(self, label, pre_clothes_mask, img_fore, clothes_mask, clothes, all_clothes_label, real_image, pose, grid, mask_fore):
        ...
        # lable - label : segmentation of human
        # pre_clothe_mask - edge : binary mask of clothes
        # img_force - img_fore : human foreground except image's background
        # clothes_mask - mask_clothes : clothes part of human parsing segmentation
        # clothes - color : color image of clothes
        # all_clothes_label - all_clothe_label : (Left / Right Arm + noise ->(to) Clothes part labels's number)
        # real_image - image : human's image
        # pose - pose : human's pose
        # grid - humans image
        # mask_fore - mask_fore : fill 1 in human parts / 0 in background parts
